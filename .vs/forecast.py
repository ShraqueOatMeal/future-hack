...
def fetch_latest_stock_data(symbol: str = "NVDA", interval: str = "1day", points: int = 12) -> Dict:
    """
    ...
    try:
        ...
        data = response.json()
        if "values" in data:
            print(f"[INFO] Successfully fetched {len(data['values'])} data points for {symbol}")
            return {"status": "ok", "symbol": symbol, "values": data["values"]}
        else:
            print(f"[ERROR] No values returned for {symbol}: {data.get('message', 'Unknown error')}")
            return {"status": "error", "message": data.get("message", "No data available"), "suggestions": ["Check stock symbol", "Verify API key"]}
    except requests.exceptions.RequestException as e:
        print(f"[ERROR] Failed to fetch stock data for {symbol}: {e}")
        return {"status": "error", "message": str(e), "suggestions": ["Check stock symbol", "Verify API key", "Check internet connection"]}
...
def query_groq_for_prediction(prompt: str) -> str:
    ...
    try:
        ...
        print(f"[INFO] Grok-3 prediction: {prediction[:100]}...")  # Truncate for brevity
        return prediction
...
def run_forecast_for_topic(query: str) -> Dict:
    ...
    if not symbols:
        ...
        return {
            ...
            "suggestions": ["Include a stock symbol (e.g., AAPL)", "Try a clearer query"]
        }
    ...
    if not results:
        ...
        return {
            ...
            "suggestions": ["Check stock symbol validity", "Verify API key", "Try a different topic"]
        }
