...
class OptimizedBusinessAnalyzer:
    ...
    def _get_internal_insights(self, user_query, intent, schema, db_data=None):
        ...
        product_tables = [t for t in tables if any(k in t.lower() for k in ['product', 'item', 'component'])]
        material_tables = [t for t in tables if 'material' in t.lower()]
        employee_tables = [t for t in tables if any(k in t.lower() for k in ['employee', 'staff', 'worker'])]
        customer_tables = [t for t in tables if any(k in t.lower() for k in ['customer', 'client', 'feedback', 'rating'])]
        ...
        if intent in self.analyzers and not db_data:
            ...
            if result.get("success", False):
                ...
                insights["reasoning"][intent.lower()] = result.get("reasoning", "Analysis performed by dedicated module")
                ...
                insights["reasoning"][intent.lower()] = result.get("reasoning", "No reasoning provided")
            ...
            if result.get("success", False):
                insights["metrics"][intent.lower()] = result.get("analysis", {})
                insights["reasoning"][intent.lower()] = result.get("reasoning", "Analysis performed on pre-fetched data")
                if "trend" in result.get("analysis", {}):
                    insights["trends"][intent.lower()] = result["analysis"]["trend"]
            else:
                insights["alerts"].append(result.get("error", "Analysis failed"))
    ...
    def _calculate_production_capacity(self, capacity_data, bottleneck_data, user_query):
        ...
        try:
            ...
            if capacity_data and isinstance(capacity_data, list):
                total_units = sum(row.get('quantity', 0) for row in capacity_data if isinstance(row.get('quantity'), (int, float)))
                capacity["available_units"] = total_units
                capacity["limiting_factor"] = "component inventory"
            if bottleneck_data and isinstance(bottleneck_data, list) and bottleneck_data:
                limiting_item = bottleneck_data[0].get('name', 'unknown')
                limiting_quantity = bottleneck_data[0].get('quantity', 0)
                capacity["limiting_factor"] = f"{limiting_item} (quantity: {limiting_quantity})"
            return capacity
        except Exception as e:
    ...
    def _calculate_trends(self, metrics):
        ...
        for metric_name, data in metrics.items():
            if isinstance(data, list) and len(data) > 1:
                key_field = 'revenue' if 'revenue' in data[0] else 'quantity' if 'quantity' in data[0] else None
                if key_field:
                    values = [row[key_field] for row in data if isinstance(row[key_field], (int, float))]
